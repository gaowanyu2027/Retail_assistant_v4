...
| `02-eval.png` | **80 条断言式评测跑通** | 终端里的评测输出（通过数/用例 id 列表） | `python evals/run_evals.py`（需 MySQL + LLM Key） |
| `03-ci.png` | **CI 双门禁** | GitHub Actions 页面（`unit` + `eval` 两个 job 都绿）或终端 `python tests/run_tests.py` 的 `PASS 123/123` | 推代码后打开仓库 Actions 页；或本地跑单测截图 |
